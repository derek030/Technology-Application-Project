...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle Vaccine Record File Upload
    if (isset($_FILES['vaccineRecordFile'])) {
        $vaccineRecordFileName = $_FILES['vaccineRecordFile']['name'];
        $vaccineRecordTempName = $_FILES['vaccineRecordFile']['tmp_name'];
        $vaccineRecordUploadPath = 'vaccine_records/' . $vaccineRecordFileName;

        if (move_uploaded_file($vaccineRecordTempName, $vaccineRecordUploadPath)) {
            // File uploaded successfully
            echo "Vaccine Record uploaded successfully.";
        } else {
            // File upload failed
            echo "Failed to upload Vaccine Record.";
        }
    }

    // Handle Pet Photo File Upload
    if (isset($_FILES['petPhotoFile'])) {
        $petPhotoFileName = $_FILES['petPhotoFile']['name'];
        $petPhotoTempName = $_FILES['petPhotoFile']['tmp_name'];
        $petPhotoUploadPath = 'pet_photos/' . $petPhotoFileName;

        if (move_uploaded_file($petPhotoTempName, $petPhotoUploadPath)) {
            // File uploaded successfully
            echo "Pet Photo uploaded successfully.";
        } else {
            // File upload failed
                ...
                <form class="space-y-6 needs-validation" novalidate>
                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
						...
						<h3 class="block leading-9 tracking-tight text-darkblue-primary font-poppins">
                                Photo</h3>
						<label for="petImage" class="block text-sm font-medium leading-6 text-darkblue-primary font-poppins">Pet Image</label>
							<div class="flex flex-row w-full space-x-4 border-dashed border-2 border-darkblue-40 px-8 py-4 rounded items-center">
								<input type="file" id="petImage" name="petImage" accept=".jpg, .jpeg, .png" />
									<div>
										<p class="font-poppins font-bold">Choose file</p>
										<p class="font-poppins">JPG or PNG, file size no more than 10MB</p>
									</div>
							</div>
						</div>
                        <div class="sm:col-span-full">
						<h3 class="block leading-9 tracking-tight text-darkblue-primary font-poppins">
                                Vaccination Certificate</h3>
						<label for="vaccineImage" class="block text-sm font-medium leading-6 text-darkblue-primary font-poppins">Vaccine Record</label>
						<div class="flex flex-row w-full space-x-4 border-dashed border-2 border-darkblue-40 px-8 py-4 rounded items-center">
							<input type="file" id="vaccineImage" name="vaccineImage" accept=".jpg, .jpeg, .png, .pdf" />
								<div>
									<p class="font-poppins font-bold">Choose file</p>
